/**
 * \file
 *         Hop-by-hop in-tree reliable data aggregation
 * \author
 *         Adam Wiktor
 */
...
struct data_msg_val {
  int32_t value;
  uint8_t counter, aggregate;
};

struct data_msg_parent {
  uint8_t is_parent;
};
...
#define RESEND_FLOOD_PERIOD 60 * CLOCK_SECOND

/* Forward declarations. */
static void send_queued_packet(struct aggregate_conn *c);
static void retransmit_callback(void *ptr);
static void retransmit_not_sent_callback(void *ptr);
static void set_keepalive_timer(struct aggregate_conn *c);
static int collect_send_aggregated(struct aggregate_conn *tc, struct data_msg_val *val);
static int has_all_values(struct aggregate_conn *c, struct data_msg_val *val);

static void
send_flood(struct aggregate_conn *c) {
  packetbuf_copyto(c->packet);
  packetbuf_clear();
  packetbuf_set_datalen(sprintf(packetbuf_dataptr(), "%s", aggregates_names[c->aggregate]) + 1);
  netflood_send(&c->flood, c->aggregate_seqno);
  packetbuf_copyfrom(c->packet, PACKETBUF_SIZE);
}

static void
resend_flood(void *ptr) {
  struct aggregate_conn *c = ptr;
  struct data_msg_val unused;
  if (c->has_value && !has_all_values(c, &unused)) {
    send_flood(c);
    ctimer_set(&c->resend_flood, RESEND_FLOOD_PERIOD, resend_flood, c);
  }
}

static int
switch_aggregate(struct aggregate_conn *c, const char *new_aggregate, uint8_t aggregate_seqno) {
  uint8_t new_aggr = NUM_AGGR;
  int i;
  for (i = 0; i < NUM_AGGR; i++)
    if (strcmp(new_aggregate, aggregates_names[i]) == 0)
      new_aggr = i;

  if (new_aggr == NUM_AGGR) {
    PRINTF("Wrong new aggregate: `%s`\n", new_aggregate);
    new_aggr = c->aggregate;
  }

  if (new_aggr != c->aggregate) {
    PRINTF("Switching aggregate to: `%s`\n", new_aggregate);
    c->aggregate = new_aggr;
    c->aggregate_seqno = aggregate_seqno;
    c->has_value = 0;
    if (collect_neighbor_list_has_children(&c->neighbor_list)) {
      collect_neighbor_list_reset_values(&c->neighbor_list);
      ctimer_set(&c->resend_flood, RESEND_FLOOD_PERIOD, resend_flood, c);
    }
    return 1;
  }
...
static int32_t
count_result(struct data_msg_val *val) {
  switch (val->aggregate) {
    case AGGR_SUM:
    case AGGR_COUNT:
    case AGGR_MAX:
    case AGGR_MIN:
      return val->value;
    case AGGR_AVG:
      return val->value / val->counter;
    default:
    PRINTF("Unknown aggregate: %d\n", val->aggregate);
    return 0;
  }
}

static int
value_changed(struct aggregate_conn *c, struct data_msg_val *val) {
  int32_t result = count_result(val);
  return !c->has_result || c->result != result;
}

static int
has_all_values(struct aggregate_conn *c, struct data_msg_val *val) {
  PRINTF("Checking all values\n");
  if (!c->has_value) {
    PRINTF("Doesnt have own value\n");
    return 0;
  }
  if (collect_neighbor_list_has_all_values(&c->neighbor_list, &val->value, &val->counter, c->aggregate)) {
    PRINTF("Has all values from children: value %ld counter %d\n", val->value, val->counter);
    switch (c->aggregate) {
      case AGGR_SUM:
      case AGGR_AVG:
        val->value += c->value;
      ...
      case AGGR_MIN:
        if (val->counter == 0)
          ...
          val->value = val->value <= c->value ? val->value : c->value;
        break;
      case AGGR_MAX:
        if (val->counter == 0)
          val->value = c->value;
    ...
    PRINTF("Has all values collected, value %ld counter %d\n", val->value, val->counter);
    return 1;
  }
  PRINTF("Doesnt have value from children\n");
  return 0;
}

static void
add_value(struct aggregate_conn *c, struct collect_neighbor *n) {
  struct data_msg_val val;
  memcpy(&val, packetbuf_dataptr(), sizeof(struct data_msg_val));
  PRINTF("Saving value %ld counter %d aggregate %d for neighbor %d.%d\n", val.value, val.counter, val.aggregate, n->addr.u8[0], n->addr.u8[1]);
  if (val.aggregate == c->aggregate) {
    if (collect_neighbor_value_same_as_previous(n, val.value, val.counter)) {
      PRINTF("Same value as before\n");
      return;
    }
    collect_neighbor_update_value(n, val.value, val.counter);
    if (has_all_values(c, &val) && value_changed(c, &val)) {
      collect_send_aggregated(c, &val);
    }
  } else {
    send_flood(c);
  }
...
static void
handle_change_parent(struct aggregate_conn *c, struct collect_neighbor *n) {
  struct data_msg_parent parent;
  memcpy(&parent, packetbuf_dataptr(), sizeof(struct data_msg_parent));
  if (parent.is_parent) {
    PRINTF("Update is parent for node %d.%d, value %d\n", n->addr.u8[0], n->addr.u8[1], parent.is_parent);
  } else {
    PRINTF("Update is no longer parent for node %d.%d, value %d\n", n->addr.u8[0], n->addr.u8[1], parent.is_parent);
  }
  collect_neighbor_set_is_child(n, parent.is_parent);
}
 ...
 */
static uint16_t
rtmetric_compute(struct aggregate_conn *tc)
{
 ...
 */
static void
bump_advertisement(struct aggregate_conn *c)
{
...
static void
inform_some_parent(struct aggregate_conn *tc, struct collect_neighbor *some_parent) {
  ...
  packetbuf_set_addr(PACKETBUF_ADDR_RECEIVER, &some_parent->addr);
  packetbuf_set_attr(PACKETBUF_ATTR_HOPS, 1);
  packetbuf_set_attr(PACKETBUF_ATTR_TTL, MAX_HOPLIM);
  packetbuf_set_attr(PACKETBUF_ATTR_MAX_REXMIT, MAX_REXMITS / 4);
...
static void
inform_new_parent(struct aggregate_conn *tc, struct collect_neighbor *new_parent) {
  struct data_msg_parent parent;
  parent.is_parent = 1;
  packetbuf_clear();
  memcpy(packetbuf_dataptr(), &parent, sizeof(struct data_msg_parent));
  packetbuf_set_datalen(sizeof(struct data_msg_parent));
  PRINTF("Informing new parent %d.%d\n", new_parent->addr.u8[0], new_parent->addr.u8[1]);
  inform_some_parent(tc, new_parent);
}

static void
inform_old_parent(struct aggregate_conn *tc, struct collect_neighbor *old_parent) {
  struct data_msg_parent parent;
  parent.is_parent = 0;
  packetbuf_clear();
  memcpy(packetbuf_dataptr(), &parent, sizeof(struct data_msg_parent));
  packetbuf_set_datalen(sizeof(struct data_msg_parent));
  PRINTF("Informing old parent %d.%d\n", old_parent->addr.u8[0], old_parent->addr.u8[1]);
  inform_some_parent(tc, old_parent);
}
 ...
 */
static void
update_parent(struct aggregate_conn *tc)
{
 ...
 */
static void
update_rtmetric(struct aggregate_conn *tc)
{
...
/*---------------------------------------------------------------------------*/
static int
enqueue_dummy_packet(struct aggregate_conn *c, int rexmits)
{
  struct collect_neighbor *n;
...
/*---------------------------------------------------------------------------*/
static void
send_packet(struct aggregate_conn *c, const linkaddr_t *receiver)
{
  clock_time_t time;
  ...
  unicast_send(&c->unicast_conn, receiver);
}
/*---------------------------------------------------------------------------*/
static void
 ...
 */
static void
send_queued_packet(struct aggregate_conn *c)
{
  struct queuebuf *q;
  struct collect_neighbor *n;
  const linkaddr_t *receiver = NULL;
  struct packetqueue_item *i;
 ...
 */
static void
retransmit_current_packet(struct aggregate_conn *c)
{
  struct queuebuf *q;
  struct collect_neighbor *n;
  struct packetqueue_item *i;
...
/*---------------------------------------------------------------------------*/
static void
send_next_packet(struct aggregate_conn *tc)
{
...
/*---------------------------------------------------------------------------*/
static void
handle_ack(struct aggregate_conn *tc)
{
  struct ack_msg msg;
  struct collect_neighbor *n;
...
/*---------------------------------------------------------------------------*/
static void
send_ack(struct aggregate_conn *tc, const linkaddr_t *to, int flags)
{
  struct ack_msg *ack;
...
/*---------------------------------------------------------------------------*/
static void
add_packet_to_recent_packets(struct aggregate_conn *tc)
{
...
{
  struct aggregate_conn *tc = (struct aggregate_conn *)
    ((char *)c - offsetof(struct aggregate_conn, unicast_conn));
  int i;
  struct data_msg_hdr hdr;
  uint8_t ackflags = 0;
  struct collect_neighbor *n;
  ...
  if(packetbuf_attr(PACKETBUF_ATTR_PACKET_TYPE) ==
    ...
    if (packetbuf_datalen() == sizeof(struct data_msg_hdr) + sizeof(struct data_msg_parent)) {
      struct queuebuf *q;
      ...
      handle_change_parent(tc, n);
      return;
    }

    /* If we are the sink, the packet has reached its final destination. */
    if(tc->rtmetric == RTMETRIC_SINK) {
      struct queuebuf *q;
...
/*---------------------------------------------------------------------------*/
static void
timedout(struct aggregate_conn *tc)
{
  struct collect_neighbor *n;
...
{
  struct aggregate_conn *tc = (struct aggregate_conn *)
    ((char *)a - offsetof(struct aggregate_conn, announcement));
  struct collect_neighbor *n;
...
static int
flood_recv(struct netflood_conn *nf, const linkaddr_t *from,
		     const linkaddr_t *originator, uint8_t seqno, uint8_t hops)
{
  struct aggregate_conn *tc = (struct aggregate_conn *)
    ((char *)nf - offsetof(struct aggregate_conn, flood));
  int ret = seqno != tc->aggregate_seqno;
  PRINTF("Flood packet with content `%s` received from %d.%d hops %d seqno %d\n",
    (char *)packetbuf_dataptr(),
    from->u8[0], from->u8[1],
    hops, seqno);

  switch_aggregate(tc, (char *)packetbuf_dataptr(), seqno);
  return ret;
}

static void
flood_sent(struct netflood_conn *c) {
  PRINTF("Flood sent\n");
}

static void
flood_drop(struct netflood_conn *c) {
  PRINTF("Flood dropped\n");
}

static const struct netflood_callbacks rreq_callbacks = {flood_recv, flood_sent, flood_drop};

void
aggregate_open(struct aggregate_conn *tc, uint16_t channels, uint8_t is_router,
	     const struct aggregate_callbacks *cb)
{
  unicast_open(&tc->unicast_conn, channels + 1, &unicast_callbacks);
  netflood_open(&tc->flood, CLOCK_SECOND * 1, channels + 2, &rreq_callbacks);
  channel_set_attributes(channels + 1, attributes);
  tc->rtmetric = RTMETRIC_MAX;
  tc->cb = cb;
  tc->seqno = 10;
  tc->eseqno = 0;
  tc->aggregate = AGGR_SUM;
  tc->aggregate_seqno = 255;
  tc->has_value = 0;
  tc->has_result = 0;
  ...
  if(!is_router) {
    PRINTF("This implementation does not support no router flag\n");
  }
...
/*---------------------------------------------------------------------------*/
static void
set_keepalive_timer(struct aggregate_conn *c)
{
...
/*---------------------------------------------------------------------------*/
void
aggregate_close(struct aggregate_conn *tc)
{
  netflood_close(&tc->flood);
...
/*---------------------------------------------------------------------------*/
void
aggregate_set_sink(struct aggregate_conn *tc, int should_be_sink)
{
  if(should_be_sink) {
    tc->rtmetric = RTMETRIC_SINK;
    PRINTF("aggregate_set_sink: tc->rtmetric %d\n", tc->rtmetric);
    bump_advertisement(tc);
...
/*---------------------------------------------------------------------------*/
static int
collect_send_aggregated(struct aggregate_conn *tc, struct data_msg_val *val) {
  struct collect_neighbor *n;
  int ret;
  int32_t result;
  packetbuf_clear();
  memcpy(packetbuf_dataptr(), val, sizeof(struct data_msg_val));
  packetbuf_set_datalen(sizeof(struct data_msg_val));
  result = count_result(val);
  tc->has_result = 1;
  tc->result = result;
  printf("[DS2019 A2] In-tree %s: %ld\n", aggregates_names[val->aggregate], result);
  packetbuf_set_attr(PACKETBUF_ATTR_EPACKET_ID, tc->eseqno);
  ...
  if(tc->rtmetric == RTMETRIC_SINK) {
    if(tc->cb->recv != NULL) {
      tc->cb->recv(aggregates_names[val->aggregate], result);
    }
    return 1;
  } else {
...
int
aggregate_send(struct aggregate_conn *tc, int32_t value)
{
  struct data_msg_val val;
  if (tc->has_value && tc->value == value) {
    PRINTF("We have already sent value %ld\n", value);
    return 1;
  }

  tc->value = value;
  tc->has_value = 1;
  if (has_all_values(tc, &val) && value_changed(tc, &val)) {
    return collect_send_aggregated(tc, &val);
  }
  return 1;
}
/*---------------------------------------------------------------------------*/
const linkaddr_t *
aggregate_parent(struct aggregate_conn *tc)
{
  return &tc->current_parent;
}
/*---------------------------------------------------------------------------*/
void
collect_purge(struct aggregate_conn *tc)
{
...
/*---------------------------------------------------------------------------*/
void
aggregate_set_aggregate(struct aggregate_conn *c, const char *aggregate) {
  static uint8_t counter = 0;
  PRINTF("Sending flood, counter %d\n", counter);
  if (!switch_aggregate(c, aggregate, counter)) {
    PRINTF("This aggregate is already set\n");
  }
  packetbuf_clear();
  packetbuf_set_datalen(sprintf(packetbuf_dataptr(), "%s", aggregate) + 1);
  netflood_send(&c->flood, counter);
  counter++;
}
/*---------------------------------------------------------------------------*/
/** @} */
